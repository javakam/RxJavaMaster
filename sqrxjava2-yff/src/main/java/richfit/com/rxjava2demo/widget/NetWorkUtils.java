...
public class NetWorkUtils {
     ...
     * 判断网址是否有效
     */
    private static Pattern CHECK_LINK_OK = Pattern.compile("^(http://|https://)?((?:[A-Za-z0-9]+-[A-Za-z0-9]+|[A-Za-z0-9]+)\\.)+([A-Za-z]+)[/\\?\\:]?.*$", Pattern.CASE_INSENSITIVE);
    ...
    public static boolean isLinkAvailable(String link) {
        Matcher matcher = CHECK_LINK_OK.matcher(link);
        if (matcher.matches()) {
            return true;
        }
        return false;
